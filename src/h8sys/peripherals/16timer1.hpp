...
inline void setPrescaler(clockSource_t ps) {
  eommpsys::H8Reg::ITU1.TCR.BIT.TPSC = calcClockSource(ps);
}
/// 現在のプリスケーラ設定の読み出し
inline clockSource_t getPreScaler(void) {
  clockSource_t ret_val;

  switch (eommpsys::H8Reg::ITU1.TCR.BIT.TPSC) {
    case 0x00:
...
inline void setClockEdge(clockEdge_t ce) {
  eommpsys::H8Reg::ITU1.TCR.BIT.CKEG = calcClockEdge(ce);
}
/// 外部クロックの入力エッジ設定状況の取得 (0: 立ち上がり 1: 立ち下がり 2:両エッジ)
inline uint8_t getClockEdge(void) {
  clockEdge_t ret_val;
  switch (eommpsys::H8Reg::ITU1.TCR.BIT.CKEG) {
    case 0x00:
...
inline void setCounterClearCause(counterClearTrigger_t cause) {
  eommpsys::H8Reg::ITU1.TCR.BIT.CCLR = calcCounterClearCause(cause);
}
/// 設定中のカウンタクリア要因の取得
