...
inline void setPrescaler(clockSource_t ps) {
  h8mmpsys::H8Reg::ITU2.TCR.BIT.TPSC = calcClockSource(ps);
}
/// 現在のプリスケーラ設定の読み出し
inline clockSource_t getPreScaler(void) {
  clockSource_t ret_val;

  switch (h8mmpsys::H8Reg::ITU2.TCR.BIT.TPSC) {
    case 0x00:
...
inline void setClockEdge(clockEdge_t ce) {
  h8mmpsys::H8Reg::ITU2.TCR.BIT.CKEG = calcClockEdge(ce);
}
/// 外部クロックの入力エッジ設定状況の取得 (0: 立ち上がり 1: 立ち下がり
/// 2:両エッジ)
inline uint8_t getClockEdge(void) {
  clockEdge_t ret_val;
  switch (h8mmpsys::H8Reg::ITU2.TCR.BIT.CKEG) {
    case 0x00:
...
inline void setCounterClearCause(counterClearTrigger_t cause) {
  h8mmpsys::H8Reg::ITU2.TCR.BIT.CCLR = calcCounterClearCause(cause);
}
/// 設定中のカウンタクリア要因の取得
...
inline void setIOModeA(IOMode_t mode) {
  h8mmpsys::H8Reg::ITU2.TIOR.BIT.IOA = calcIOReg(mode);
}
/// GRBの入出力、TIORB端子の機能設定
inline void setIOModeB(IOMode_t mode) {
  h8mmpsys::H8Reg::ITU2.TIOR.BIT.IOB = calcIOReg(mode);
}
/// タイマー出力レベルの設定
inline void setTimerPinLevelA(bool state) {
  h8mmpsys::H8Reg::ITU.TOLR.BIT.TOA2 = state;
}
/// タイマー出力レベルの設定
inline void setTimerPinLevelB(bool state) {
  h8mmpsys::H8Reg::ITU.TOLR.BIT.TOB2 = state;
}
/// 位相計測モードの設定(true: 位相計測モード, false: 通常モード)
inline void setPhaseMeasureMode(bool state) {
  h8mmpsys::H8Reg::ITU.TMDR.BIT.MDF = state;
}
/// TISRCのOVFフラグのセット条件を設定(true:
/// 16TCNT2がオーバーフローしたときに1にセット, false:
/// 16TCNT2がオーバーフローもしくはアンダーフロー)
inline void setTriggerOVF(bool state) {
  h8mmpsys::H8Reg::ITU.TMDR.BIT.FDIR = state;
}
