...
/// AD変換開始要求のトリガーを選択(No_Source(禁止), External_Fall,
/// Compare_MatchA)
inline void setADtrigger(clockSource_t trigger) {
  h8mmpsys::H8Reg::TMR0.TCSR.BIT.ADTE = calcADtrigger(trigger);
}

/// CMFAによる割り込みの認可状況
inline bool intrruptStatsA(void) {
  return h8mmpsys::H8Reg::TMR0.TCSR.BIT.CMFA;
}
/// CMFAのリセット
inline void resetIntrFlagA(void) {
  h8mmpsys::H8Reg::TMR0.TCSR.BIT.CMFA = 0;
}
/// CMFAによる割り込みの設定(true: 許可 false: 禁止)
inline void permitIntrA(bool flag) {
  h8mmpsys::H8Reg::TMR0.TCR.BIT.CMIEA = flag;
}
/// CMFAによる割り込みの認可状況
inline bool getPermissionIntrA(void) {
  return h8mmpsys::H8Reg::TMR0.TCR.BIT.CMIEA;
}
/// TCORBによるコンペアマッチ・インプットキャプチャの発生を示す
inline bool intrruptStatsB(void) {
  return h8mmpsys::H8Reg::TMR0.TCSR.BIT.CMFB;
}
/// CMFBのリセット
inline void resetIntrFlagB(void) {
  h8mmpsys::H8Reg::TMR0.TCSR.BIT.CMFB = 0;
}
/// CMFBによる割り込みの設定(true: 許可 false: 禁止)
inline void permitIntrB(bool flag) {
  h8mmpsys::H8Reg::TMR0.TCR.BIT.CMIEB = flag;
}
/// CMFAによる割り込みの認可状況
inline bool getPermissionIntrB(void) {
  return h8mmpsys::H8Reg::TMR0.TCR.BIT.CMIEB;
}
/// TCORBによるコンペアマッチ・インプットキャプチャの発生を示す
inline bool intrruptStatsOVF(void) {
  return h8mmpsys::H8Reg::TMR0.TCSR.BIT.OVF;
}
/// CMFBのリセット
inline void resetIntrFlagOVF(void) {
  h8mmpsys::H8Reg::TMR0.TCSR.BIT.OVF = 0;
}
/// CMFBによる割り込みの設定(true: 許可 false: 禁止)
inline void permitIntrOVF(bool flag) {
  h8mmpsys::H8Reg::TMR0.TCR.BIT.OVIE = flag;
}
/// CMFAによる割り込みの認可状況
inline bool getPermissionIntrOVF(void) {
  return h8mmpsys::H8Reg::TMR0.TCR.BIT.OVIE;
}

/// TCORAを取得する
inline uint8_t getTCORA(void) {
  return h8mmpsys::H8Reg::TMR0.TCORA;
}
/// TCORAに代入する
inline void setTCORA(uint8_t val) {
  h8mmpsys::H8Reg::TMR0.TCORA = val;
}
/// TCORBを取得する
inline uint8_t getTCORB(void) {
  return h8mmpsys::H8Reg::TMR0.TCORB;
}
/// TCORBに代入する
inline void setTCORB(uint8_t val) {
  h8mmpsys::H8Reg::TMR0.TCORB = val;
}
/// クロックソース選択レジスタの値を計算
...
inline void setClockSource(clockSource_t cs) {
  h8mmpsys::H8Reg::TMR0.TCR.BIT.CKS = calcClockSource(cs);
}
/// 現在のクロックソース設定の読み出し
inline clockSource_t getClockSource(void) {
  clockSource_t ret_val;
  switch (h8mmpsys::H8Reg::TMR0.TCR.BIT.CKS) {
    case 0x00:
...
inline void setCounterClearCause(counterClearTrigger_t cause) {
  h8mmpsys::H8Reg::TMR0.TCR.BIT.CCLR = calcCounterClearCause(cause);
}
/// 設定中のカウンタクリア要因の取得
inline uint8_t getCounterClearCause(void) {
  counterClearTrigger_t ret_val;
  switch (h8mmpsys::H8Reg::TMR0.TCR.BIT.CCLR) {
    case 0x00:
...
inline void setIOModeA(IOMode_t mode) {
  h8mmpsys::H8Reg::TMR0.TCSR.BIT.OS = calcIOMode(mode);
}

inline void setIOModeB(IOMode_t mode) {
  h8mmpsys::H8Reg::TMR0.TCSR.BIT.OIS = calcIOMode(mode);
}
/// タイマーカウントをスタート
